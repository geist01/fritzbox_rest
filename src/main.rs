...
fn read_config() -> Result<Config> {
    ...
    let matches = App::new("AVM Fritzbox API")
        ...
        .subcommand(SubCommand::with_name("temperature")
                    ...
                    .arg(Arg::with_name("ain")
                         ...
                         .long("ain")
                         .value_name("AIN")))
        .subcommand(SubCommand::with_name("switchon")
                    .arg(Arg::with_name("sid")
                         .takes_value(true)
                         .short("s")
                         .long("sid")
                    ...
                    .arg(Arg::with_name("ain")
                         ...
                         .long("ain")
                         .value_name("AIN")))
        .subcommand(SubCommand::with_name("switchoff")
                    .arg(Arg::with_name("sid")
                         .takes_value(true)
                         .short("s")
                         .long("sid")
                    ...
                    .arg(Arg::with_name("ain")
                         ...
                         .long("ain")
                         .value_name("AIN")))
        .subcommand(SubCommand::with_name("switchstate")
                    .arg(Arg::with_name("sid")
                         .takes_value(true)
                         .short("s")
                         .long("sid")
    ...
    let mut config = match Ini::load_from_file("config/defaults.ini") {
        ...
        Err(_) => {
	    return Err(fritzbox_rest::errors::ErrorKind::MissingParameter("host / psw are mandatory".to_string()).into());
        }
    };
    ...
    match matches.subcommand() {
        ("test", Some(m)) => {
            ...
            config.lng = m.value_of("lng").map(|s| s.parse::<f64>().unwrap()).or(config.lng);
            config.lat = m.value_of("lat").map(|s| s.parse::<f64>().unwrap()).or(config.lat);
            config.host_mobile = m.value_of("host_mobile").map(|s| s.to_string()).or(config.host_mobile);
            config.port_mobile = m.value_of("port_mobile").map(|s| s.to_string()).or(config.port_mobile);
        },
        ("timer", Some(m)) => {
