...
pub fn fetch_sid(config : &Config, client : &Client) -> Result<String> {
    ...
    let v : Vec<u16> = format!("{0}-{1}", session_info.challenge, &config.psw).encode_utf16().collect();
	...
	None => Url::parse_with_params(&url, &[("response", format!("{0}-{1:#?}", session_info.challenge, digest)),]),
	Some(user) => Url::parse_with_params(&url, &[("response", format!("{0}-{1:#?}", session_info.challenge, digest)), ("username", user.to_string())])
    } {
...
pub fn list_devices(client : &Client, config : &Config, sid : &str) -> Result<DeviceListInfos> {
    let url = Url::parse_with_params(
        &format!("{0}://{1}/webservices/homeautoswitch.lua", config.protocol, config.host),
...
pub fn read_switchstate(client : &Client, config : &Config, sid : &str) -> Result<bool> {
    let url = Url::parse_with_params(
        &format!("{0}://{1}/webservices/homeautoswitch.lua", config.protocol, config.host),
        &[("sid", sid), ("ain", &config.ain.clone().unwrap()), ("switchcmd", "getswitchstate")])?;
...
pub fn read_temperature(client : &Client, config : &Config, sid : &str) -> Result<usize> {
    let url = Url::parse_with_params(
        &format!("{0}://{1}/webservices/homeautoswitch.lua", config.protocol, config.host),
        &[("sid", sid), ("ain", &config.ain.clone().unwrap()), ("switchcmd", "gettemperature")])?;
...
pub fn read_consumption(client : &Client, config : &Config, sid : &str) -> Result<f64> {
    let url = Url::parse_with_params(
        &format!("{0}://{1}/webservices/homeautoswitch.lua", config.protocol, config.host),
        &[("sid", sid), ("ain", &config.ain.clone().unwrap()), ("switchcmd", "getswitchpower")])?;
...
pub fn set_switch_on_off(client : &Client, config : &Config, sid : &str, on : bool) -> Result<()> {
    ...
    let url = Url::parse_with_params(
        &format!("{0}://{1}/webservices/homeautoswitch.lua", config.protocol, config.host),
        &[("sid", sid), ("ain", &config.ain.clone().unwrap()), ("switchcmd", command)])?;
